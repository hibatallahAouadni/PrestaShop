/**
 * Created by hibatallah.aouadni on 06/01/18.
 */
...
scenario('Create a customer account in the Front Office', client => {
    ...
    test('should set the "First name" input', () => client.waitAndSetValue(accountPage.firstname_input, 'John'));
    test('should set the "Last name" input', () => client.waitAndSetValue(accountPage.lastname_input, 'Doe'));
    test('should set the "Email" input', () => client.waitAndSetValue(accountPage.email_input, 'pub' + date_time + '@prestashop.com'));
    test('should set the "Password" input', () => client.waitAndSetValue(accountPage.password_input, '123456789'));
...
scenario('Create "Address"', client => {
    ...
    test('should set the "Address" input', () => client.waitAndSetValue(accountPage.adr_address, '16, Main street'));
    test('should set the "Zip/Postal Code" input', () => client.waitAndSetValue(accountPage.adr_postcode, '75002'));
...
scenario('Check the creation of the address', client => {
    ...
    test('should check that the "First name" of customer is equal to "John"', () => client.checkAttributeValue(accountPage.firstname_input, 'value', 'John'));
    test('should check that the "Last name" of customer is equal to "Doe"', () => client.checkAttributeValue(accountPage.lastname_input, 'value', 'Doe'));
    test('should check that the "Address" of customer is equal to "16, Main street"', () => client.checkAttributeValue(accountPage.adr_address, 'value', '16, Main street'));
    test('should check that the "Zip/Postal Code" of customer is equal to "75002"', () => client.checkAttributeValue(accountPage.adr_postcode, 'value', '75002'));
    test('should check that the "City" of customer is equal to "Paris"', () => client.checkAttributeValue(accountPage.adr_city, 'value', 'Paris'));
